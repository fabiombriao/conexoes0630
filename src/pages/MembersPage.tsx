...
const MemberProfile: React.FC<{ profile: any }> = ({ profile }) => {
  ...
  return (
    <div>
      {/* Cover / Header */}
      <div className="h-28 bg-gradient-to-br from-primary/50 via-primary/20 to-accent/30 rounded-t-lg relative" />

      <div className="px-5 pb-6 -mt-12">
        {/* Avatar + Name */}
        <div className="flex flex-col items-center text-center mb-4">
          <div className="h-24 w-24 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-4xl ring-4 ring-card mb-3">
            {profile.avatar_url ? (
          ...
          {profile.company_name && (
            <p className="text-sm text-primary font-medium">{profile.company_name}</p>
          )}
          {profile.business_category && (
            <span className="mt-1 text-xs bg-primary/10 text-primary px-3 py-1 rounded-full">
              {profile.business_category}
            </span>
          )}
        </div>
        ...
        <div className="flex justify-center gap-3 mb-5">
          {whatsappHref && (
            <a href={whatsappHref} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-primary hover:bg-primary/80 text-primary-foreground text-sm font-medium transition-colors">
          ...
          {profile.instagram_url && (
            <a href={profile.instagram_url} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-accent hover:bg-accent/80 text-accent-foreground text-sm font-medium transition-colors">
            ...
            <div className="grid gap-2">
              {gainsFields.map((f) => (
                <div key={f.key} className="bg-muted/20 border border-border rounded-lg p-3">
                  ...
                  <p className="text-sm text-foreground/90 leading-relaxed">{profile[f.key]}</p>
                </div>
              ))}
            </div>
          </div>
