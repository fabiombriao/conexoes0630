...
// Windows-1252 chars (codepoint → original byte) that appear when UTF-8 bytes
// 0x80–0x9F are mis-decoded via CP1252 instead of Latin-1.
...
const fixMojibake = (str: string): string => {
  ...
  try {
    const bytes = new Uint8Array([...str].map((c) => {
      ...
      const byte = CP1252_TO_BYTE[code] ?? code;
      if (byte > 255) throw new Error();
      return byte;
    }));
